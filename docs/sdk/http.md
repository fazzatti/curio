...
For normal applications, import from `@curio/sdk/http/oak`.
...
The root `@curio/sdk` entrypoint stays transport-agnostic.
...
That metadata is currently consumed by `@curio/sdk/openapi`.
...
annotate it with `withSchemas(...)` from `@curio/sdk`:

```ts
import * as v from "@valibot/valibot";
import { withSchemas } from "@curio/sdk";
