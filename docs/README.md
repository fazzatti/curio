...
{% hint style="info" %} **Beta Software** — Curio is still in `0.x.x`. Public
APIs may change between minor releases while the package boundaries and happy
path continue to settle. {% endhint %}

Curio is a TypeScript-first backend toolkit for Deno. It combines typed route
authoring, a relational-first data layer, a server-rendered admin runtime, and a
bootstrap CLI that generates a Curio project from the canonical template.

## Why Curio?

- **One backend, one admin surface** — Mount the admin in the same backend that
  owns your APIs and persistence layer.
- **Small happy-path API** — Start with Oak, Valibot, and the generated project
  template instead of assembling a framework from scratch.
- **Focused extension points** — Drop to adapter-agnostic HTTP, build artifacts,
  schema metadata, or admin modules only when you need more control.
- **Project generation that stays honest** — `@curio/init` produces the same
  project shape Curio itself expects and tests, including first-time setup for
  the generated admin.
...
| `@curio/init` | Bootstrap CLI that scaffolds a new Curio project from the canonical template and optional scaffold features |
