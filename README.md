...
- [`packages/core`](./packages/core): the reusable framework package, intended for
  JSR publication as `@curio/core`
...
- `packages/core` is the package closest to publishability
- `packages/init/template/default/` is the canonical generated app template
- `packages/core/src/OLD-reference/` is legacy prototype material and should not
  be treated as current architecture
- `docs/` is the canonical documentation tree for GitBook and repository docs

The immediate goal is to harden and publish the core package first, then
rewire `packages/init` against the published package.
