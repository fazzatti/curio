...
- `@curio/core/admin/modules/sessions`
  - built-in admin session/auth helpers
- `@curio/core/admin/modules/audit`
...
```ts
import { API, Route } from "@curio/core/http/oak";
...
Curio's testing helpers live under `@curio/core/testing`.
...
Curio's value-object base lives under `@curio/core/value-object`.
...
`@curio/core/admin` also exports the built-in admin/auth models:
...
import { resolveAdminActor } from "@curio/core/admin/modules/sessions";
import { recordAdminAuditEvent } from "@curio/core/admin/modules/audit";
```
...
This split is intentional. The root core entrypoint stays lean for HTTP-only and
